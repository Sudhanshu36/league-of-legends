...
match.events <- data.match %>%
  ...
  mutate(participantId = ifelse(participantId <= 5, 100, 200),
         ...
         teamId = ifelse(teamId == 100, 200, 100)) # associate to non-victim! 
...
Interactive version.
```{r}
low <- min(match.events$matchId)
high <- max(match.events$matchId)

shinyUI(fluidPage(
fluidRow(
column(4, selectInput("eventType", "Type:", 
                      c(unique(as.character(match.events$eventType))), selected = "BUILDING_KILL")),
column(4, sliderInput("matchId", "Match ID:", 
                      min = low, max = high, value = low, ticks = FALSE))
),
fluidRow(
renderPlot({
    data <- match.events %>%
