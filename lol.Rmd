...
```{r}
first.blood <- data.team %>%
  filter(firstBlood == TRUE) %>%
  select(winner) %>%
  summarise(prob = sum(winner)/n() * 100) %>%
  mutate(eventType = "First Blood")

#   sum(winner)/n() * 100
# 1                  61.1

first.dragon <- data.team %>%
  filter(firstDragon == TRUE) %>%
  select(winner) %>%
  summarise(prob = sum(winner)/n() * 100) %>%
  mutate(eventType = "First Dragon")

#   sum(winner)/n() * 100
# 1               69.4864

first.tower <- data.team %>%
  filter(firstTower == TRUE) %>%
  select(winner) %>%
  summarise(prob = sum(winner)/n() * 100) %>%
  mutate(eventType = "First Tower")

#   sum(winner)/n() * 100
# 1                  68.2

first.herald <- data.team %>%
  filter(firstRiftHerald == TRUE) %>%
  select(winner) %>%
  summarise(prob = sum(winner)/n() * 100) %>%
  mutate(eventType = "First Herald")

#   sum(winner)/n() * 100
# 1               74.4898

first.baron <- data.team %>%
  filter(firstBaron == TRUE) %>%
  select(winner) %>%
  summarise(prob = sum(winner)/n() * 100) %>%
  mutate(eventType = "First Baron")

#   sum(winner)/n() * 100
# 1              82.53425

first.inhibitor <- data.team %>%
  filter(firstInhibitor == TRUE) %>%
  select(winner) %>%
  summarise(prob = sum(winner)/n() * 100) %>%
  mutate(eventType = "First Inhibitor")

#   sum(winner)/n() * 100
# 1              90.59081

first <- rbind(first.blood, first.tower, first.dragon, first.herald, first.baron) # , first.inhibitor)
first <- transform(first, eventType = reorder(eventType, prob))
first$color <- c(TRUE, TRUE, TRUE, TRUE, FALSE)

p <- ggplot(first, aes(eventType, prob, fill = color)) +
...
```{r}
library(prob)
count.winherald <- data.team %>%
  group_by(winner, firstRiftHerald) %>%
  summarise(probs = n()) %>%
  probspace(count.winherald$probs)

count.winherald %>%
  Prob(event = winner == TRUE, given = firstRiftHerald == TRUE)
...
herald <- count.winherald %>%
  filter(firstRiftHerald == TRUE) %>%
  as.data.frame() %>%
  select(count) %>%
  summarise(count = sum(count))

winherald <- count.winherald %>%
  filter(firstRiftHerald == TRUE & winner == TRUE) %>%
  as.data.frame() %>%
  select(count) %>%
  summarise(count = sum(count))

require(venneuler)
v <- venneuler(c(Win = win$count, Herald = herald$count, "Win&Herald" = winherald$count))
v <- venneuler(c(A = 1000, B = 294, "A&B" = 219))
plot(v)

# library(VennDiagram)
# draw.pairwise.venn(1000,294,219)
```
