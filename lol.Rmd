...
duration.median <- data.match %>%
  spread_values(matchDuration = jnumber("matchDuration")) %>%
  mutate(matchDuration = matchDuration/60) %>%
  use_series(matchDuration) %>%
  median()
...
match.monster <- match.events %>%
  ...
  select(-monsterType) %>%
  mutate(eventType = str_replace("BARON_NASHOR", "BARON"),
         eventType = str_replace("RIFTHERALD", "HERALD"))

# Compute the median time to draw first blood
match.kill <- match.events %>%
...
match.maxlvl <- match.events %>%
  ...
  as.data.frame() %>%
  summarise(timestamp = median(timestamp)) %>%
  mutate(eventType = "MAX LVL")

match <- rbind(match.monster, match.duration, match.kill, match.maxlvl)
match <- match[order(match$timestamp),]
...
ggplot(match) + 
  ...
  expand_limits(x = match.duration) +
...
math.surrender <- match.events %>%
  ...
  select(matchId, teamId, winner, NEXUS_TURRET) %>%
