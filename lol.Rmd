...
if (method == "rf") {
  ...
  modelGrid <- expand.grid(nrounds = seq(2, 10, by = 2), lambda = seq(2, 5, by = 0.25), 
                           alpha = seq(1, 6, by = 0.5))
}
...
rocCurve.best <- as.data.frame(t(coords(rocCurve, "best", 
...
p <- rocCurve.data %>%
  ...
  geom_line(color = roccol, alpha = 0.5) +
...
#   mutate(class = ifelse("red" > thresold, "red", "blue")) %>%
...
```{r}
CM <- confusionMatrix(modelPredict, testing$winner, positive = "red")

# Turn accuracy information into data frame
CM.df <- data.frame(t(CM$overall))
CM.df
(CM.df$AccuracyUpper - CM.df$AccuracyLower)/2

# Plot accuracy with error bars
ggplot(CM.df, aes("", Accuracy)) +
