...
data <- participants %>%
  ...
  mutate(creepsPerMinDeltas = ifelse(teamId == 100, -1, 1) * creepsPerMinDeltas,
         ...
         winner = ifelse(winner, teamId, 300-teamId)) %>%
  group_by(matchId, winner) %>%
  summarise_each(funs(sum))

ggplot(data, aes(goldPerMinDeltas, xpPerMinDeltas, 
                 color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
...
data <- events %>% 
  ...
  merge(participants) %>%
  mutate(teamId = as.numeric(as.character(teamId))) %>%
  mutate(CHAMPION_KILL = ifelse(as.numeric(as.character(teamId)) == 100, -1, 1) * CHAMPION_KILL,
         creepsPerMinDeltas = ifelse(teamId == 100, -1, 1) * creepsPerMinDeltas,
         goldPerMinDeltas = ifelse(teamId == 100, -1, 1) * goldPerMinDeltas,
         xpPerMinDeltas = ifelse(teamId == 100, -1, 1) * xpPerMinDeltas,
         winner = ifelse(winner, teamId, 300-teamId)) %>%
  group_by(matchId, winner) %>%
...
We prepare data frame for farming per type per minute.
```{r}
