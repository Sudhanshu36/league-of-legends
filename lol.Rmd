...
column(4, selectInput("eventType", "Type:", 
                      c("All", unique(as.character(match.events$eventType))), 
...
# Show and save plot
print(p)
ggsave("figures/events.png", width = 10, height = 10, units = "in")
```
...
renderPlot({
      ...
      if (input$eventType != "All") {
        data <- data %>%
          dplyr::filter(eventType == input$eventType)
      }
  
      if (input$team == "Blue") {
        data <- data %>%
          dplyr::filter((teamId == 100 & winner == TRUE) | (teamId == 200 & winner == FALSE))
      } else if (input$team == "Red") {
        data <- data %>%
          dplyr::filter((teamId == 100 & winner == FALSE) | (teamId == 200 & winner == TRUE))
      }
...
timeline.tower <- match.events %>% 
  dplyr::filterfilter(eventType == "BUILDING_KILL") %>%
  select(matchId, timestamp) %>%
  group_by(matchId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filterfilter(rank == 1) %>%
...
timeline.inhibitor <- match.events %>% 
  dplyr::filterfilter(buildingType == "INHIBITOR_BUILDING") %>%
  select(matchId, timestamp) %>%
  group_by(matchId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filterfilter(rank == 1) %>%
...
timeline.monster <- match.events %>%
  dplyr::filterfilter(eventType == "ELITE_MONSTER_KILL") %>%
...
timeline.kill <- match.events %>%
  dplyr::filterfilter(eventType == "CHAMPION_KILL") %>% 
  select(matchId, timestamp) %>%
  group_by(matchId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filterfilter(rank == 1) %>%
...
ggplot(melt(CM.count), aes(predict, true, fill = value)) + geom_raster()
