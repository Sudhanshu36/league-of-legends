...
map.draw <- function(to, tf) {
  ...
  p <- coord %>%
    ...
    # dplyr::filter((teamId == 100 & winner == TRUE) | (teamId == 200 & winner == FALSE)) %>%
    dplyr::filter((teamId == 100 & winner == FALSE) | (teamId == 200 & winner == TRUE)) %>%
    make_map + annotate("text", x = 500, y = 10, label = as.character(to))
...
ggsave("figures/methods-acc.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```
...
ggsave("figures/methods-roc.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```
