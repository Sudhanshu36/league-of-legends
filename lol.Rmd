---
title: "League of Legends Data Analysis"
author: "Vincent Quenneville-Bélair"
date: "March, 2016"
runtime: shiny
output: html_document
---

Loading packages.
```{r}
library(gridExtra)
library(png)
library(grid)
library(animation)
library(ggplot2)
library(rattle)
library(shiny)

library(prob)
library(caret)
library(pROC)

library(stringr)

# https://cran.r-project.org/web/packages/tidyjson/vignettes/introduction-to-tidyjson.html
# https://github.com/sailthru/tidyjson
library("tidyjson")

library(reshape2)
library(plyr)
library(magrittr)
library(tidyr)
library(dplyr)
```

```{r}
# Extract gg color
gg_color_hue <- function(n) {
  hues = seq(15, 375, length=n+1)
  hcl(h=hues, l=65, c=100)[1:n]
}

gg_red  <- gg_color_hue(4)[1] # red: F8766D
gg_green <- gg_color_hue(4)[2]
gg_blue <- gg_color_hue(4)[3] # blue: #00BFC4
gg_purple <- gg_color_hue(4)[4]
```

```{r}
# Read RIOT API key
# riot.key <- readLines("riot.key")
# options("riot.key" = riot.key)
```

Load match file.
```{r}
# TidyJSON
# data.file <- "data/matches1.json"
data.file <- "data/matches_all.json"
data.json <- read_json(data.file)
```

Load match data.
```{r}
data.match <- data.json %>% 
  gather_array("file.index") %>%
  enter_object("matches") %>%
  gather_array("matches.index") %>% 
  spread_values(matchId = jnumber("matchId")) %>%
  select(matchId) 

match.num <- data.match %>%
  summarise(count = n())

# Print general match information
data.match %>%
  spread_values(mapId = jnumber("mapId"),
                matchDuration = jnumber("matchDuration"),
                matchMode = jstring("matchMode"),
                matchType = jstring("matchType"),
                queueType = jstring("queueType"),
                region = jstring("region"),
                season = jstring("season")) %>%
  head()
```

Match duration
```{r}
match.duration <- data.match %>%
  spread_values(matchDuration = jnumber("matchDuration")) %>%
  mutate(matchDuration = matchDuration/60) 

match.duration %>%
  select(matchDuration) %>%
  summary

#  matchDuration  
# Min.   :12.82  
# 1st Qu.:26.93  
# Median :32.25  
# Mean   :32.59  
# 3rd Qu.:37.33  
# Max.   :59.62  
```

## Look into events

```{r}
# data.match %>%
#   enter_object("timeline") %>%
#   enter_object("frames") %>%
#   gather_array("frames.index") %>% 
#   enter_object("participantFrame") %>% str()
#   gather_array("participantFrames.index") %>% str()
#   spread_values(participantId = jnumber(participantId))
```

We make a data frame for events.
```{r}
events.all <- data.match %>%
    enter_object("timeline") %>%
    enter_object("frames") %>%
    gather_array("frames.index") %>%
    enter_object("events") %>%
    gather_array("events.index") %>%
    select(matchId) %>%
    spread_values(eventType = jstring("eventType"),
                  timestamp = jnumber("timestamp"),
                  teamId = jnumber("teamId"),
                  participantId = jnumber("participantId"), 
                  creatorId = jnumber("creatorId"),
                  victimId = jnumber("victimId"),
                  killerId = jnumber("killerId"),
                  monsterType = jstring("monsterType"),
                  towerType = jstring("towerType"),
                  buildingType = jstring("buildingType")) %>%
    mutate(timestamp = timestamp/1000/60) 

# Beware: going deeper picks only the rows with positions available
events.coord <- events.all %>%
    enter_object("position") %>%
    spread_values(X = jnumber("x"),
                  Y = jnumber("y"))

events.all <- merge(events.all, events.coord, all.x = TRUE)
```

```{r}
# Show how to access information about participants in a frame.
data.match %>%
  enter_object("timeline") %>%
  enter_object("frames") %>% 
  gather_array("frames.index") %>% 
  enter_object("participantFrames") %>% 
  enter_object("3")
```

## Conditional Probabilities

We make the table of winner per match.
```{r}
# Extract per team information
data.first <- data.match %>%
  enter_object("teams") %>%
  gather_array("teams.index") %>% 
  spread_values(
    teamId = jnumber("teamId"),
    winner = jlogical("winner"),
    firstTower = jlogical("firstTower"),
    firstBaron = jlogical("firstBaron"),
    firstBlood = jlogical("firstBlood"),
    firstDragon = jlogical("firstDragon"),
    firstInhibitor = jlogical("firstInhibitor"),
    firstRiftHerald = jlogical("firstRiftHerald")) %>%
  select(-teams.index)

# Show a little more stat
data.first %>%
  spread_values(
    towerKills = jnumber("towerKills"), 
    riftHeraldKills = jnumber("riftHeraldKills"), 
    firstTower = jnumber("firstTower"),
    inhibitorKills = jnumber("inhibitorKills"), 
    dragonKills = jnumber("dragonKills"), 
    baronKills = jnumber("baronKills")) %>%
  head()

#      matchId teamId winner firstTower firstBaron firstBlood firstDragon firstInhibitor firstRiftHerald towerKills riftHeraldKills
# 1 2055016505    100   TRUE      FALSE       TRUE       TRUE        TRUE           TRUE           FALSE         10               0
# 2 2055016505    200  FALSE       TRUE      FALSE      FALSE       FALSE          FALSE           FALSE          4               0
# 3 2055016716    100   TRUE       TRUE      FALSE       TRUE        TRUE           TRUE           FALSE         11               0
# 4 2055016716    200  FALSE      FALSE      FALSE      FALSE       FALSE          FALSE           FALSE          1               0
# 5 2055016717    100  FALSE      FALSE      FALSE      FALSE       FALSE          FALSE           FALSE          3               0
# 6 2055016717    200   TRUE       TRUE      FALSE       TRUE        TRUE           TRUE           FALSE          9               0
#   firstTower.1 inhibitorKills dragonKills baronKills
# 1            0              1           4          1
# 2            1              0           0          1
# 3            1              3           3          0
# 4            0              0           0          0
# 5            0              0           2          0
# 6            1              1           2          0

match.winner <- data.first %>%
  select(matchId, teamId, winner)

team.probspace <- probspace(data.first)

Prob(team.probspace, event = winner == TRUE, given = teamId == 200)
Prob(team.probspace, event = winner == TRUE, given = teamId == 100)

# [1] 0.492
# [1] 0.508
```

Proporitions having various events.
```{r}
data.first %>%
  filter(winner == TRUE) %>%
  select(-winner) %>%
  gather(eventType, WinHave, -matchId, -teamId) %>%
  ggplot(aes(x = eventType, fill = WinHave)) +
  geom_bar()
```

Conditional probabilities on various objectives
```{r}
first.blood <- data.frame(eventType = "First Champion", 
                          probs = Prob(team.probspace, event = winner == TRUE, 
                                       given = firstBlood == TRUE))
first.dragon <- data.frame(eventType = "First Dragon", 
                           probs = Prob(team.probspace, event = winner == TRUE, 
                                        given = firstDragon == TRUE))
first.tower <- data.frame(eventType = "First Tower", 
                          probs = Prob(team.probspace, event = winner == TRUE, 
                                       given = firstTower == TRUE))
first.herald <- data.frame(eventType = "First Herald", 
                           probs = Prob(team.probspace, event = winner == TRUE, 
                                        given = firstRiftHerald == TRUE))
first.baron <- data.frame(eventType = "First Baron", 
                          probs = Prob(team.probspace, event = winner == TRUE, 
                                       given = firstBaron == TRUE))
first.inhibitor <- data.frame(eventType = "First Inhibitor", 
                              probs = Prob(team.probspace, event = winner == TRUE, 
                                           given = firstInhibitor == TRUE))

first <- rbind(first.blood, first.tower, first.dragon, first.herald, first.baron) # , first.inhibitor)
first <- transform(first, eventType = reorder(eventType, probs))
first$color <- c(TRUE, TRUE, TRUE, TRUE, FALSE)

#      eventType    probs color
# 1  First Blood 61.10000  TRUE
# 2  First Tower 68.20000  TRUE
# 3 First Dragon 69.48640  TRUE
# 4 First Herald 74.48980  TRUE
# 5  First Baron 82.53425 FALSE

p <- ggplot(first, aes(eventType, probs, fill = color)) +
  # expand_limits(y = c(.5,1)) +
  geom_bar(stat = "identity") +
  scale_fill_discrete(guide = 'none') +
  coord_cartesian(ylim = c(.5, .85), expand = TRUE) +
  xlab("") +
  ylab("Probability of winning given ...") +
  geom_hline(yintercept = .5, color = gg_purple)
  # geom_vline(xintercept = 4.5)
  # geom_vline(xintercept = (which(first$eventType == 'First Herald') + which(first$eventType == 'First Baron'))/2)

print(p)
ggsave("figures/conditional.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

Plot Venn diagram for winner and first Rift Herald.
```{r}
win <- Prob(team.probspace, event = winner == TRUE)
winANDherald <- Prob(team.probspace, event = winner == TRUE & firstRiftHerald == TRUE)
herald <- Prob(team.probspace, event = firstRiftHerald == TRUE)

win
winANDherald
herald

require(venneuler)
v <- venneuler(c(Win = win - winANDherald, 
                 Herald = herald - winANDherald, 
                 "Win&Herald" = winANDherald))

plot(v, col = c(gg_blue, gg_red))
png("figures/venn.png")
```

We reformat the data with each row being one match and goal to predict winning team.
```{r} 
match.first <- data.first %>%
  mutate(winner = winner * teamId,
         firstBaron = firstBaron * teamId,
         firstBlood = firstBlood * teamId,
         firstDragon = firstDragon * teamId,
         firstInhibitor = firstInhibitor * teamId,
         firstTower = firstTower * teamId,
         firstRiftHerald = firstRiftHerald * teamId) %>%
  group_by(matchId) %>%
  summarise_each(funs(sum)) %>%
  select(-teamId) %>%
  mutate_each(funs(factor)) %>%
  select(winner,
         matchId,
         # firstBaron,
         # firstInhibitor,
         firstBlood,
         firstDragon,
         firstTower,
         firstRiftHerald)
```

## Surrender 

Proportion of matches that ends in surrender. A team who lost and still had at least one Nexus tower surrendered. We assume all surrenders are as such.
```{r}
# Extract tower information to infer whether losing team surrendered
match.turret <- events.all %>%
  dplyr::filter(towerType == "NEXUS_TURRET") %>%
  mutate(NEXUS_TURRET = 1) %>%
  select(matchId, teamId, NEXUS_TURRET)

match.surrender <- merge(match.winner, match.turret, all.x = TRUE) %>%
  group_by(matchId, teamId, winner) %>%
  summarise_each(funs(sum)) %>%
  mutate(surrender = ifelse(winner, FALSE, 
                            ifelse(NEXUS_TURRET < 2 | is.na(NEXUS_TURRET), TRUE, FALSE))) %>%
  group_by(matchId) %>%
  summarise_each(funs(sum)) %>%
  select(matchId, surrender) %>%
  mutate(surrender = as.logical(surrender))

timeline.surrender <- merge(match.duration, match.surrender) %>%
  group_by(surrender) %>%
  summarise_each(funs(median)) %>%
  mutate(eventType = ifelse(surrender, "SURRENDER", "CASTLE DESTRUCTION"),
         timestamp = matchDuration) %>%
  select(timestamp, eventType)

# End if surrender
merge(match.duration, match.surrender) %>%
  filter(surrender == TRUE) %>%
  select(-surrender, -matchId) %>%
  summary()

# matchDuration  
# Min.   :20.17  
# 1st Qu.:21.43  
# Median :25.48  
# Mean   :26.99  
# 3rd Qu.:30.55  
# Max.   :51.12 

# End if not surrender
merge(match.duration, match.surrender) %>%
  filter(surrender == FALSE) %>%
  select(-surrender, -matchId) %>%
  summary()

# matchDuration  
# Min.   :12.82  
# 1st Qu.:29.20  
# Median :33.42  
# Mean   :34.01  
# 3rd Qu.:38.38  
# Max.   :59.62  

# Percent of matches that ended in surrender?
x <- match.surrender$surrender
print(sum(x) / length(x) * 100)
```

## Activity versus time

```{r}
events.team <- events.all %>%
  mutate(participantId = ifelse(participantId <= 5, 100, 200),
         creatorId = ifelse(creatorId <= 5, 100, 200),
         victimId = ifelse(victimId <= 5, 200, 100),   # associate non-victim team!
         killerId = ifelse(killerId <= 5, 100, 200),
         teamId = ifelse(teamId == 100, 200, 100)  # associate to non-victim team!
         ) %>%
  # Fill in team association
  mutate(victimId      = ifelse(is.na(victimId), killerId, victimId)) %>%
  mutate(creatorId     = ifelse(is.na(creatorId), victimId, creatorId)) %>%
  mutate(participantId = ifelse(is.na(participantId), creatorId, participantId)) %>%
  mutate(teamId        = ifelse(is.na(teamId), participantId, teamId)) %>%
  select(-participantId, -creatorId, -victimId, -killerId) %>%
  # Attach winning status
  merge(match.winner, all.x = TRUE) %>%
  mutate(eventType = as.factor(eventType),
         teamId = as.factor(teamId))

# Save for presentation
write.csv(as.data.frame(events.team), file = "data/events.csv")
```
  
Plot activity per few minutes normalized per match.
```{r}
binwidth <- 5

# Divide events in bins
bin.events <- events.team %>%
  dplyr::filter(eventType == "CHAMPION_KILL") %>%
  # dplyr::filter(eventType == "ELITE_MONSTER_KILL") %>%
  mutate(bin = timestamp %/% binwidth) %>%
  group_by(winner, bin) %>%
  summarise(count = n())

# Divide matches in bins -- count in last full bin
bin.match <- match.duration %>% 
  mutate(bin = matchDuration %/% binwidth) %>%
  group_by(bin) %>%
  summarise(match = n())

bin.match <- rbind(data.frame(bin = c(0, 1), match = c(0, 0)), bin.match) %>%
  mutate(remain = sum(match) - cumsum(match))

bin <- merge(bin.events, bin.match, all.x = TRUE) %>%
  mutate(countPerMatch = ifelse(remain == 0, NA, count / remain)) %>%
  select(bin, countPerMatch, winner) %>%
  na.omit()

bin.win <- bin %>%
  filter(winner == TRUE) %>%
  select(-winner)
bin.lost <- bin %>%
  filter(winner == FALSE) %>%
  select(-winner)

# Choice of position: dodge, identity, stack, fill, jitter
p <- ggplot(bin, aes(bin+0.5, countPerMatch, group = winner, fill = winner)) +
  geom_bar(alpha = 0.5, stat = "identity", position = "identity") +
  scale_x_continuous(breaks = seq(0, max(bin$bin), 2), 
                   labels = seq(0, binwidth*max(bin$bin), 2*binwidth)) +
  # coord_cartesian(xlim = c(0, max(bin$bin)+1), expand = TRUE) +
  ylab("events per match") +
  xlab("minutes")

plot(p)
ggsave("figures/activity.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

Plot activity per few minutes.
```{r}
# Choice of position: dodge, identity, stack, fill, jitter
p <- events.team %>%
  # dplyr::filter(eventType == "CHAMPION_KILL") %>%
  dplyr::filter(eventType == "ELITE_MONSTER_KILL") %>% 
  ggplot(aes(timestamp, y = ..count.., group = winner, fill = winner)) + 
  xlab("minutes") +
  stat_bin(binwidth = 5, alpha = 0.7, col = I("white"), position = "identity")

# p <- ggplot(data, aes(timestamp, 
#                       y = (..count..)/sum(..count..), group = winner, fill = winner)) + 
#   xlab("minutes") +
#   ylab("proportion for each team") +
#   stat_bin(binwidth = 5, col = I("white"), position = "fill")

plot(p)
# ggsave("figures/activity.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

## Map

We transform the coordinate given in data frame to coordinate on the image.
```{r}
# We need to apply a linear transformation 
# mapping the given coordinates to the corresponding pixels on the image

# Coordinate range in data frame
# min: {x: 0, y: 0}
# max: {x: 14820, y: 14881}
# Coordinate range on map
# min: {x: -570, y: -420}
# max: {x: 15220, y: 14980}
# Image pixel coordinates
# min: {x: 0, y: 0}
# max: {x: 512, y: 512}

f.x <- function(x) {
  # Input range -570 to 15220
  # Output range 0 to 512
  
  i.xmin = -570
  i.xmax = 15220
  o.xmin = 0
  o.xmax = 512
  
  o.xmin + (o.xmax - o.xmin) * (x - i.xmin) / (i.xmax - i.xmin)
}

f.y <- function(y) {
  # Input range -420 to 14980
  # Output range 0 to 512
  
  i.ymin = -420
  i.ymax = 14980
  o.ymin = 0
  o.ymax = 512
  
  o.ymin + (o.ymax - o.ymin) * (y - i.ymin) / (i.ymax - i.ymin)
}

# Extract the positions of all events in a given match
# ... and apply transformation to find corresponding pixel on image
coord <- events.team %>% 
  mutate(X = f.x(X), Y = f.y(Y))

# Save for presentation
write.csv(as.data.frame(coord), file = "data/map.csv")
```

We plot the events of a match over the map.
```{r}
make_map <- function(data) {
  
  # Get background image
  img <- readPNG("data/minimap-mh-alpha.png")
  g <- rasterGrob(img, interpolate = TRUE, width = unit(1,"npc"), height = unit(1,"npc"))
  
  # Scatter plot with background
  p <- data %>%
    ggplot(aes(X, Y, color = teamId, group = teamId)) + 
    scale_color_manual(na.value = "black", values = c(gg_blue, gg_red)) +
    annotation_custom(g, xmin = 0, xmax = 512, ymin = 0, ymax = 512) +
    expand_limits(x = c(0,512), y = c(0,512))
  
  # p <- p + geom_point(alpha = 0.1, size = 1)
  
  # p <- p + stat_density2d(aes(fill = ..level..), geom = "polygon", alpha = 0.2)
  # p <- p + stat_density2d(aes(alpha = ..level..))
  p <- p + stat_density2d(size = 1, alpha = 0.7)
  # p <- p + geom_bin2d(stat = "contour")
  # p <- p + geom_bin2d(aes(size = ..count..), stat = "contour")
  # p <- p + stat_contour(na.rm = TRUE)
  
  # Remove padding and decorations
  p <- p +
    scale_y_continuous(expand = c(0,0)) + 
    scale_x_continuous(expand = c(0,0)) +
    theme(aspect.ratio = 1) +
    theme(axis.line = element_blank(),
          axis.text.x = element_blank(),
          axis.text.y = element_blank(),
          axis.ticks = element_blank(),
          axis.title.x = element_blank(),
          axis.title.y = element_blank(),
          legend.position = "none",
          panel.background = element_blank(),
          panel.border = element_blank(),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          plot.background = element_blank())
}

# Pick matches to map
data <- coord %>%
  # dplyr::filter((teamId == 100 & winner == TRUE) | (teamId == 200 & winner == FALSE))
  dplyr::filter((teamId == 100 & winner == FALSE) | (teamId == 200 & winner == TRUE))

# Select one match only
# data <- coord %>%
#   dplyr::filter(matchId == 2054994244)

# Leave buildings as a separate color
# data <- data %>%
#   mutate(teamId = ifelse(eventType == "BUILDING_KILL", NA, teamId)) 

# Show and save plot
p <- make_map(data)
print(p)
ggsave("figures/map.png", width = 10, height = 10, units = "in", bg = "transparent")
```

Make animated GIF.
```{r}
map.draw <- function(to, tf, print.label = TRUE) {
  timeslice <- c(to, tf)

  p <- coord %>%
    dplyr::filter(timeslice[1] < timestamp & timestamp < timeslice[2]) %>%
    # dplyr::filter((teamId == 100 & winner == TRUE) | (teamId == 200 & winner == FALSE)) %>%
    dplyr::filter((teamId == 100 & winner == FALSE) | (teamId == 200 & winner == TRUE)) %>%
    make_map 
  
  if (print.label == TRUE) {
    p <- p + annotate("text", x = 500, y = 10, label = as.character(to))
  }

  print(p)
}

# Show the first 20 minutes.
map.draw(0, 20, print.label = FALSE)
ggsave("figures/map_20.png", width = 10, height = 10, units = "in", bg = "transparent")

map.animate <- function() {
  d <- 5
  lapply(seq(0, 45, d), function(i) {
    map.draw(i, i+d)
  })
}

# Make an animation from time slices
saveGIF(map.animate(), interval = .65, movie.name = "map.gif")
```

## Timeline

When are certain objectives achieved for the time in a match?
```{r}
# Length of games
match.duration %>%
  select(matchDuration) %>%
  summary

timeline.end <- match.duration %>%
  summarise(timestamp = median(matchDuration)) %>%
  mutate(eventType = "END")
```

Compute median time when first tower is destroyed
```{r}
timeline.tower <- events.all %>% 
  dplyr::filter(eventType == "BUILDING_KILL") %>%
  select(matchId, timestamp) %>%
  group_by(matchId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filter(rank == 1) %>%
  as.data.frame() %>%
  select(timestamp)

summary(timeline.tower)

#   timestamp     
# Min.   : 5.124  
# 1st Qu.:11.097  
# Median :12.712  
# Mean   :12.868  
# 3rd Qu.:14.563  
# Max.   :22.169  

timeline.tower <- timeline.tower %>%
  summarise_each(funs(median)) %>%
  mutate(eventType = "TOWER DESTRUCTION")
```

Compute median time when first tower is destroyed
```{r}
timeline.inhibitor <- events.all %>% 
  dplyr::filter(buildingType == "INHIBITOR_BUILDING") %>%
  select(matchId, timestamp) %>%
  group_by(matchId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filter(rank == 1) %>%
  as.data.frame() %>%
  select(timestamp)

summary(timeline.inhibitor)

timeline.inhibitor <- timeline.inhibitor %>%
  summarise_each(funs(median)) %>%
  mutate(eventType = "INHIBITOR DESTRUCTION")
```

Compute median time when first elite monster is killed
```{r}
timeline.monster <- events.all %>%
  dplyr::filter(eventType == "ELITE_MONSTER_KILL") %>%
  select(matchId, timestamp, monsterType) %>%
  group_by(matchId, monsterType) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filter(rank == 1) %>%
  as.data.frame() 

timeline.monster %>%
  spread(monsterType, timestamp) %>% 
  select(-matchId, -rank) %>% 
  summary

#  BARON_NASHOR       DRAGON         RIFTHERALD    
# Min.   :20.20   Min.   : 4.879   Min.   : 4.534  
# 1st Qu.:26.06   1st Qu.:10.862   1st Qu.:13.378  
# Median :29.36   Median :12.929   Median :16.296  
# Mean   :29.89   Mean   :13.256   Mean   :15.552  
# 3rd Qu.:33.18   3rd Qu.:15.446   3rd Qu.:18.202  
# Max.   :44.68   Max.   :25.135   Max.   :19.824  
# NA's   :410     NA's   :1        NA's   :700  

timeline.monster <- timeline.monster %>%
  select(timestamp, monsterType) %>%
  group_by(monsterType) %>%
  summarise_each(funs(median)) %>%
  mutate(eventType = monsterType) %>%
  select(-monsterType) %>%
  mutate(eventType = str_replace(eventType, "DRAGON", "DRAGON KILL"),
         eventType = str_replace(eventType, "BARON_NASHOR", "BARON KILL"),
         eventType = str_replace(eventType, "RIFTHERALD", "HERALD KILL"))
```

Compute the median time to draw first blood
```{r}
timeline.kill <- events.all %>%
  dplyr::filter(eventType == "CHAMPION_KILL") %>% 
  select(matchId, timestamp) %>%
  group_by(matchId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filter(rank == 1) %>%
  as.data.frame() 

timeline.kill %>% 
  select(timestamp) %>%
  summary

#   timestamp    
# Min.   :0.707  
# 1st Qu.:2.630  
# Median :3.155  
# Mean   :3.329  
# 3rd Qu.:3.799  
# Max.   :9.326  

timeline.kill <- timeline.kill %>%
  summarise(timestamp = median(timestamp)) %>%
  mutate(eventType = "CHAMPION KILL")
```

Compute the median time to reach level 18
```{r}
timeline.maxlvl <- events.all %>%
  dplyr::filter(eventType == "SKILL_LEVEL_UP") %>%
  mutate(teamId = ifelse(participantId <= 5, 100, 200)) %>%
  select(matchId, timestamp, teamId) %>%
  group_by(matchId, teamId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  dplyr::filter(rank == 5*18)

timeline.maxlvl %>%
  as.data.frame %>%
  select(timestamp) %>%
  summary

#   timestamp    
# Min.   :37.60  
# 1st Qu.:41.66  
# Median :44.57  
# Mean   :44.55  
# 3rd Qu.:46.81  
# Max.   :53.46  

# Print proporitions of game reaching level 18
x <- length(unique(timeline.maxlvl$matchId))
n <- length(data.match$matchId)
print(x/n * 100)
# [1] 6.3

# Add to timeline
timeline.maxlvl <- timeline.maxlvl %>%
  as.data.frame() %>%
  summarise(timestamp = median(timestamp)) %>%
  mutate(eventType = "MAX LEVEL")

# Compute the median level reached
events.all %>%
  dplyr::filter(eventType == "SKILL_LEVEL_UP") %>%
  mutate(teamId = ifelse(participantId <= 5, 100, 200)) %>%
  select(matchId, timestamp, teamId) %>%
  group_by(matchId, teamId) %>%
  mutate(rank = rank(timestamp, na.last = TRUE, ties.method = "first")) %>%
  select(-timestamp) %>%
  summarise_each(funs(max(rank))) %>%
  as.data.frame() %>%
  summarise(rank = median(rank)/5)

#   rank
# 1 15.5
```

Plot timeline events.
```{r}
timeline <- rbind(timeline.monster, timeline.kill, timeline.maxlvl, timeline.surrender, timeline.tower)
            # timeline.inhibitor
timeline <- timeline[order(timeline$timestamp),]

#   timestamp          eventType
#       (dbl)              (chr)
# 1  3.154658      CHAMPION KILL
# 2 12.928967        DRAGON KILL
# 3 16.296100        HERALD KILL
# 4 25.475000          SURRENDER
# 5 29.355583         BARON KILL
# 6 33.416667 CASTLE DESTRUCTION
# 7 44.567367          MAX LEVEL
  
# Draw timeline
# http://stackoverflow.com/questions/7492274/draw-a-chronological-timeline-with-ggplot2

# dislocations <- sample(c(-1,-.5,.5,1), dim(timeline)[1], replace = T)
dislocations <- c(1.0, -1.0, 0.5, -0.5, 1.0, -1.0, 0.5, -0.5)
nudges <- 0.1 * sign(dislocations)

p <- ggplot(timeline) + 
  geom_text(aes(x = timestamp, y = dislocations, label = eventType), 
            position = position_nudge(y = nudges)) +
  geom_hline(yintercept = 0, color = gg_purple, size = 1) +
  geom_segment(aes(x = timestamp, y = dislocations, xend = timestamp, yend = 0)) +
  xlab("minutes") +
  scale_x_continuous(expand = c(.15, .15))

p <- p + 
  theme(panel.grid.minor.y = element_blank(),
        panel.grid.major.y = element_blank(),
        axis.line = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank(),
        axis.title.y = element_blank(),
        legend.position = "none")

print(p)
ggsave("figures/timeline.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

## Scatter plot

```{r}
tf <- 20
```

```{r}
# Delta fields refer to values for the specified period (e.g., the gold per minute over the first 10 minutes of the game versus the second 20 minutes of the game) 
# Diffs fields refer to the deltas versus the calculated lane opponent(s)

participants.timeline <- data.match %>%
  enter_object("participants") %>%
  gather_array("participants.index") %>%
  spread_values(participantId = jnumber("participantId"),
                teamId = jnumber("teamId")) %>%
  enter_object("timeline")

participantTimelineData <- function(x) {
  participants.timeline %>% 
    enter_object(x) %>%
    gather_keys("ParticipantTimelineData") %>%
    append_values_number(x)
}

participants.creeps <- participantTimelineData("creepsPerMinDeltas")
participants.gold <- participantTimelineData("goldPerMinDeltas")
# participants.wards <- participantTimelineData("wardsPerMinDeltas")
participants.xp <- participantTimelineData("xpPerMinDeltas")
# participants.kills <- participantTimelineData("assistedLaneKillsPerMinDeltas")
# participants.golem <- participantTimelineData("ancientGolemKillsPerMinCounts")
# participants.baron <- participantTimelineData("baronKillsPerMinCounts")
# participants.dragon <- participantTimelineData("dragonKillsPerMinCounts")
# participants.lizard <- participantTimelineData("elderLizardKillsPerMinCounts")
# participants.inhibitor <- participantTimelineData("inhibitorKillsPerMinCounts")
# participants.tower <- participantTimelineData("towerKillsPerMinCounts")
participants.damageTaken <- participantTimelineData("damageTakenPerMinDeltas")

participants <- merge(participants.creeps, participants.gold)
participants <- merge(participants, participants.xp)
participants <- merge(participants, participants.damageTaken)

if (tf <= 10) {
participants <- merge(participants, match.winner) %>%
  select(matchId, teamId, ParticipantTimelineData, 
         creepsPerMinDeltas, goldPerMinDeltas, xpPerMinDeltas, winner) %>%
  group_by(matchId, teamId, ParticipantTimelineData, winner) %>%
  summarise_each(funs(mean)) %>%
  dplyr::filter(ParticipantTimelineData == "zeroToTen") %>%
  as.data.frame() %>%
  select(-ParticipantTimelineData) %>%
  group_by(matchId, teamId, winner) %>%
  summarise_each(funs(mean))
} else if (tf <= 20) {
participants <- merge(participants, match.winner) %>%
  select(matchId, teamId, ParticipantTimelineData, 
         creepsPerMinDeltas, goldPerMinDeltas, xpPerMinDeltas, winner) %>%
  group_by(matchId, teamId, ParticipantTimelineData, winner) %>%
  summarise_each(funs(mean)) %>%
  dplyr::filter(ParticipantTimelineData == "zeroToTen" | 
                  ParticipantTimelineData == "tenToTwenty") %>%
  as.data.frame() %>%
  select(-ParticipantTimelineData) %>%
  group_by(matchId, teamId, winner) %>%
  summarise_each(funs(mean))
} else if (tf <= 30) {
participants <- merge(participants, match.winner) %>%
  select(matchId, teamId, ParticipantTimelineData, 
         creepsPerMinDeltas, goldPerMinDeltas, xpPerMinDeltas, winner) %>%
  group_by(matchId, teamId, ParticipantTimelineData, winner) %>%
  summarise_each(funs(mean)) %>%
  dplyr::filter(ParticipantTimelineData == "zeroToTen" | 
                  ParticipantTimelineData == "tenToTwenty") %>%
  as.data.frame() %>%
  select(-ParticipantTimelineData) %>%
  group_by(matchId, teamId, winner) %>%
  summarise_each(funs(mean))
} else {
participants <- merge(participants, match.winner) %>%
  select(matchId, teamId, ParticipantTimelineData, 
         creepsPerMinDeltas, goldPerMinDeltas, xpPerMinDeltas, winner) %>%
  group_by(matchId, teamId, ParticipantTimelineData, winner) %>%
  summarise_each(funs(mean)) %>%
  as.data.frame() %>%
  select(-ParticipantTimelineData) %>%
  group_by(matchId, teamId, winner) %>%
  summarise_each(funs(mean))
}

#       matchId teamId winner creepsPerMinDeltas goldPerMinDeltas xpPerMinDeltas
#         (dbl)  (dbl)  (lgl)              (dbl)            (dbl)          (dbl)
# 1  2054994176    100  FALSE               3.66           270.78         359.19
# 2  2054994176    200   TRUE               4.15           320.34         387.32
# 3  2054994196    100  FALSE               3.55           318.21         402.64
# 4  2054994196    200   TRUE               4.08           332.82         385.74
# 5  2054994244    100  FALSE               3.27           255.12         372.72
# 6  2054994244    200   TRUE               4.10           329.23         424.98
# 7  2054994283    100  FALSE               3.54           255.87         364.14
# 8  2054994283    200   TRUE               4.52           336.31         415.82
# 9  2054994354    100  FALSE               3.78           304.94         358.04
# 10 2054994354    200   TRUE               3.41           299.32         378.73
# ..        ...    ...    ...                ...              ...            ...

ggplot(participants, aes(goldPerMinDeltas, xpPerMinDeltas, 
                         color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
  # stat_density2d(aes(alpha = ..density..), geom = "raster", contour = FALSE)
  # geom_bin2d(aes(color = ..count..), binwidth = 5, alpha = 0.5) +
  # stat_bin2d(binwidth = 5, alpha = 0.5) +
  xlab(paste("Gold per min in first", tf, "min")) +
  ylab(paste("Experience per min in first", tf, "min"))

ggsave("figures/scatter.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

We prepare data per match for farming per type per minute.
```{r}
match.participants <- participants %>%
  as.data.frame() %>%
  mutate(creepsPerMinDeltas = ifelse(teamId == 100, -1, 1) * creepsPerMinDeltas,
         goldPerMinDeltas = ifelse(teamId == 100, -1, 1) * goldPerMinDeltas,
         xpPerMinDeltas = ifelse(teamId == 100, -1, 1) * xpPerMinDeltas) %>%
  mutate(teamId = as.numeric(as.character(teamId))) %>%
  mutate(winner = factor(ifelse(winner, teamId, 300-teamId))) %>% 
  select(-teamId) %>%
  group_by(matchId, winner) %>%
  summarise_each(funs(sum))

match.participants %>%
  ggplot(aes(goldPerMinDeltas, xpPerMinDeltas, 
             color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
  # stat_density2d(aes(alpha = ..density..), geom = "raster", contour = FALSE)
  # geom_bin2d(aes(color = ..count..), binwidth = 5, alpha = 0.5) +
  # stat_bin2d(binwidth = 5, alpha = 0.5) +
  xlab(paste("Gold per min in first", tf, "min")) +
  ylab(paste("Experience per min in first", tf, "min"))
```

We prepare data frame for number of events per type per minute.
```{r}
match.events <- events.team %>% 
  filter(timestamp < tf) %>% 
  mutate(teamId = as.numeric(as.character(teamId))) %>%
  mutate(winner = factor(ifelse(winner, teamId, 300-teamId))) %>% 
  group_by(matchId, teamId, eventType, winner) %>%
  summarise(count = n()/tf) %>%
  mutate(count = ifelse(teamId == 100, -1, 1) * count) %>%
  spread(eventType, count, fill = 0) %>%
  group_by(matchId, winner) %>%
  summarise_each(funs(sum)) %>%
  select(-teamId) %>%
  select(-ITEM_DESTROYED, -ITEM_SOLD, -ITEM_UNDO)

# events.team %>% 
#   filter(timestamp < tf) %>% 
#   mutate(teamId = as.numeric(as.character(teamId))) %>%
#   mutate(winner = factor(ifelse(winner, teamId, 300-teamId))) %>% 
#   group_by(matchId, teamId, monsterType, winner) %>%
#   summarise(count = n()/tf) %>%
#   mutate(count = ifelse(teamId == 100, -1, 1) * count) %>%
#   na.omit %>%
#   spread(monsterType, count, fill = 0) %>%
#   group_by(matchId, winner) %>%
#   summarise_each(funs(sum)) %>%
#   as.data.frame() %>%
#   select(-teamId)

# Plot number of wards.
match.events %>%
  ggplot(aes(WARD_KILL, WARD_PLACED, 
             color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
  # stat_density2d(aes(alpha = ..density..), geom = "raster", contour = FALSE)
  # geom_bin2d(aes(color = ..count..), binwidth = 5, alpha = 0.5) +
  # stat_bin2d(binwidth = 5, alpha = 0.5) +
  xlab(paste("Wards destroyed difference per min in first", tf, "min")) +
  ylab(paste("Wards placed difference per min in first", tf, "min"))
```

```{r}
match.events.team <- events.team %>% 
  filter(timestamp < tf) %>% 
  filter(eventType == "CHAMPION_KILL" |
           eventType == "BUILDING_KILL" |
           eventType == "ELITE_MONSTER_KILL" |
           eventType == "SKILL_LEVEL_UP" |
           eventType == "WARD_PLACED" |
           eventType == "ITEM_PURCHASED") %>%
  mutate(teamId = as.numeric(as.character(teamId))) %>%
  mutate(winner = factor(ifelse(winner, teamId, 300-teamId))) %>% 
  group_by(matchId, teamId, eventType, winner) %>%
  summarise(count = n()/tf) %>%
  as.data.frame() %>%
  unite(eventType.teamId, c(eventType, teamId), sep = ".") %>%
  spread(eventType.teamId, count, fill = 0)

match.monster <- events.team %>% 
  filter(timestamp < tf) %>% 
  mutate(teamId = as.numeric(as.character(teamId))) %>%
  mutate(winner = factor(ifelse(winner, teamId, 300-teamId))) %>% 
  group_by(matchId, teamId, monsterType, winner) %>%
  summarise(count = n()/tf) %>%
  na.omit() %>%
  as.data.frame() %>%
  unite(monsterType.teamId, c(monsterType, teamId), sep = ".") %>%
  spread(monsterType.teamId, count, fill = 0)

# Merge monster in place of elite monster kill
match.events.team <- match.events.team %>%
  select(-ELITE_MONSTER_KILL.100, -ELITE_MONSTER_KILL.200) %>%
  merge(match.monster, all.x = TRUE)

# Fill NAs by 0
match.events.team[is.na(match.events.team)] <- 0
```

We look at the number of wards.
```{r}
data <- events.team %>% 
  filter(timestamp < tf) %>%
  group_by(matchId, eventType, winner) %>%
  summarise(count = n()/tf) %>%
  spread(eventType, count)

ggplot(data, aes(WARD_KILL, WARD_PLACED, 
                 color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
  # stat_density2d(aes(alpha = ..density..), geom = "raster", contour = FALSE)
  # geom_bin2d(aes(color = ..count..), binwidth = 5, alpha = 0.5) +
  # stat_bin2d(binwidth = 5, alpha = 0.5) +
  xlab(paste("Wards destroyed per min in first", tf, "min")) +
  ylab(paste("Wards placed per min in first", tf, "min"))
```

We look at number of kills versus creeps.
```{r}
data <- events.team %>% 
  filter(timestamp < tf) %>%
  filter(eventType == "CHAMPION_KILL") %>%
  group_by(matchId, winner) %>%
  summarise(CHAMPION_KILL = n()/tf) %>%
  merge(participants)

ggplot(data, aes(CHAMPION_KILL, creepsPerMinDeltas, 
                 color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
  # stat_density2d(aes(alpha = ..density..), geom = "raster", contour = FALSE)
  # geom_bin2d(aes(color = ..count..), binwidth = 5, alpha = 0.5) +
  # stat_bin2d(binwidth = 5, alpha = 0.5) +
  xlab(paste("Champion killed per min in first", tf, "min")) +
  ylab(paste("Creeps killed per min in first", tf, "min"))
```

We look at number of kills versus creeps.
data <- merge(match.events, match.participants)
```{r}
ggplot(data, aes(CHAMPION_KILL, creepsPerMinDeltas, 
                 color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
  geom_density2d(alpha = 1) +
  # stat_density2d() +
  # stat_density2d(aes(alpha = ..density..), geom = "raster", contour = FALSE)
  # geom_bin2d(aes(color = ..count..), binwidth = 5, alpha = 0.5) +
  # stat_bin2d(binwidth = 5, alpha = 0.5) +
  xlab(paste("Champion killed per min in first", tf, "min")) +
  ylab(paste("Creeps killed per min in first", tf, "min"))
```

## Prediction

We first rename categories to predict.
```{r}
# data <- match.events
data <- match.events.team
# data <- match.first
# data <- merge(match.events, match.first) %>% merge(match.participants) 

# data <- match.events.team

data <- data %>%
  mutate(winner, winner = factor(ifelse(winner == 100, "blue", "red")))
```

We divide the dataset into train and test.
```{r}
set.seed(1)

training.index <- createDataPartition(data$winner, p = .8, list = FALSE)
training <- data[ training.index,]
testing  <- data[-training.index,]

training.noId <- select(training, -matchId)
testing.noId <- select(testing, -matchId)

# Enough features?
nrow(training)/ncol(training.noId)
```

We create dummy variables for categorical variables.
```{r}
# dummies <- dummyVars(~ ., data = training, sep = ".", fullRank = TRUE)
# training <- data.frame(predict(dummies, training))
# testing <- data.frame(predict(dummies, testing))
# 
# # Enough features?
# nrow(training)/ncol(training)
```

We cross-validate.
```{r}
# Select method
# List supported by Caret https://topepo.github.io/caret/modelList.html

# method <- "rf"
# method <- "rpart"
# method <- "glm"
# method <- "glmnet"
# method <- "svmLinear"
method <- "svmRadial"
# method <- "xgbLinear"
# method <- "ada"
  
# List of methods in caret: http://topepo.github.io/caret/bytag.html
method.all <- c("rf", "rpart", "glm", "glmnet", "svmLinear", "svmRadial", "xgbLinear", "ada")
# method.all <- c("rf", "rpart", "glmnet", "svmLinear", "svmRadial", "xgbLinear")

modelCV <- function(method) {

preProc = NULL
if (method == "rf") {
  modelGrid <- expand.grid(mtry = seq(1, 5, by = 1))
} else if (method == "rpart") {
  modelGrid <- expand.grid(cp = seq(0.01, 0.1, by = 0.01))
} else if (method == "glm") {
  modelGrid <- NULL
} else if (method == "glmnet") {
  # alpha between 0 (ridge) and 1 (lasso)
  modelGrid <- expand.grid(.alpha = c(0, 0.1, 0.5, 0.7, 1), .lambda = seq(0, 10, by = 0.1))
} else if (method == "svmLinear") {
  modelGrid <- expand.grid(C = seq(0.5, 5, by = 0.5))
  preProc <- c("center", "scale")
} else if (method == "svmRadial") {
  modelGrid <- expand.grid(C = seq(0.5, 5, by = 0.5), sigma = seq(0.1, 0.5, by = 0.1))
  preProc <- c("center", "scale")
} else if (method == "xgbLinear") {
  modelGrid <- expand.grid(nrounds = seq(10, 310, by = 50), 
                           lambda = seq(0.1, 3, by = 0.5), 
                           alpha = seq(0.01, 2, by = 0.5))
} else if (method == "ada") {
  modelGrid <- expand.grid(iter = seq(2, 10, by = 2), 
                           maxdepth = seq(3, 7, by = 1), 
                           nu = seq(0.001, 1, by = 0.1))
}

# Cross-validation parameters
ctrl <- trainControl(method = "cv", number = 10, verboseIter = T, classProbs = TRUE)

# We can try different methods: svmLinear glmnet rpart svmRadial
modelTune <- train(winner ~ .,
                   data = training.noId,
                   tuneGrid = modelGrid,
                   trControl = ctrl,
                   method = method,
                   preProc = preProc,
                   # summaryFunction = twoClassSummary,
                   # metric = "ROC",
                   # importance = TRUE,
                   na.action = na.pass)

}

modelTune <- modelCV(method)
modelTune

varImp(modelTune)
plot(modelTune)

# RF with 10 min
# SKILL_LEVEL_UP.200 100.0000
# BUILDING_KILL.200   94.7361
# SKILL_LEVEL_UP.100  90.6074
# BUILDING_KILL.100   87.9419
# CHAMPION_KILL.100   86.8206
# CHAMPION_KILL.200   78.8555
# ITEM_PURCHASED.100  54.3742
# WARD_PLACED.100     52.3170
# ITEM_PURCHASED.200  51.6448
# WARD_PLACED.200     51.1540
# DRAGON.200          30.4174
# DRAGON.100          28.3539
# RIFTHERALD.200       0.8256
# RIFTHERALD.100       0.0000

# Rf with 20 min
# SKILL_LEVEL_UP.200 100.0000
# BUILDING_KILL.200   94.7361
# SKILL_LEVEL_UP.100  90.6074
# BUILDING_KILL.100   87.9419
# CHAMPION_KILL.100   86.8206
# CHAMPION_KILL.200   78.8555
# ITEM_PURCHASED.100  54.3742
# WARD_PLACED.100     52.3170
# ITEM_PURCHASED.200  51.6448
# WARD_PLACED.200     51.1540
# DRAGON.200          30.4174
# DRAGON.100          28.3539
# RIFTHERALD.200       0.8256
# RIFTHERALD.100       0.0000

modelTune$bestTune
plot(modelTune$results)

# Plot tree
par(mfrow = c(1,1))
# plot(modelTune$finalModel)
fancyRpartPlot(modelTune$finalModel)
```

We evaluate using the ROC curve.
```{r}
probsTrain <- predict(modelTune,
                      training.noId,
                      type = "prob",
                      na.action = na.pass)

rocCurve   <- roc(response = training$winner,
                  predictor = probsTrain$red,
                  levels = rev(levels(training$winner)))

# names(rocCurve)

# Choose threshold according to sensitivity or specificity
# qplot(x = rocCurve$thresholds, y = rocCurve$sensit)
# qplot(x = rocCurve$thresholds, y = rocCurve$spec)

# Plot ROC curve
# plot(rocCurve, print.thres = "best")

# Plot true positive rate (sensitivity) versus false positive rate (1-specificity)
rocCurve.data <- data.frame(FPR = 1-rocCurve$specificities, TPR = rocCurve$sensitivities)
rocCurve.best <- rocCurve %>%
  coords("best", ret = c("sensitivity", "specificity", "threshold")) %>%
  t() %>%
  as.data.frame()

roccol <- gg_red

rocCurve.data %>%
  ggplot(aes(x = FPR, y = TPR)) + 
  geom_point(color = roccol, size = 0.5) +
  geom_line(color = roccol, alpha = 0.5, size = 0.5) +
  # geom_smooth(color = roccol, se = FALSE, alpha = 0.25) +
  geom_point(data = rocCurve.best, aes(x = 1-specificity, y = sensitivity), 
             color = gg_purple, size = 3) +
  geom_text(data = rocCurve.best, aes(x = 1-specificity, y = sensitivity, 
                                      label = round(threshold, 3)), hjust = 1, vjust = -1) +
  xlab("False Positive Rate") +
  ylab("True Positive Rate") +
  # geom_abline(intercept = 0, slope = 1, alpha = 0.5, color = gg_purple)
  geom_segment(x = 0, y = 0, xend = 1, yend = 1, color = gg_purple, alpha = 0.5, size = 0.25)

# Set thresold for division between the two categories
rocCurve.thresold <- rocCurve.best$threshold
modelTrain <- probsTrain %>%
  mutate(winner = ifelse(red > rocCurve.thresold, "red", "blue")) %>%
  mutate(winner = as.factor(winner))

# Check AUC
rocCurve.auc <- as.numeric(rocCurve$auc)
rocCurve.auc
```

We look at misclassified.
```{r}
training.misclassified <- training %>%
  cbind(data.frame(misclassified = modelTrain$winner != training$winner))

training.misclassified %>%
  merge(match.duration) %>%
  ggplot(aes(matchDuration, fill = misclassified)) + 
  geom_histogram(binwidth = 2)
```

```{r}
ggplot(training.misclassified, aes(CHAMPION_KILL, ELITE_MONSTER_KILL, 
                         # color = winner, group = winner)) +
                         color = misclassified, group = misclassified)) + 
  # geom_point(size = 2) +
  # geom_point(aes(size = misclassified), alpha = 0.5) +
  # geom_point(aes(shape = misclassified), alpha = 0.5, size = 3) +
  # geom_point(aes(alpha = misclassified), size = 2) +
  geom_point(size = 2) +
  scale_color_manual(values = c(gg_blue, gg_red)) +
  # geom_density2d(alpha = 1) +
  xlab(paste("Champion killed per min in first", tf, "min")) +
  ylab(paste("Elite monster killed per min in first", tf, "min"))

# ggplot(testing.misclassified, aes(firstBlood, firstDragon, 
#                          color = misclassified, group = misclassified)) + 
#   geom_point(size = 2, position = "jitter") +
#   scale_color_manual(values = c(gg_blue, gg_red)) +
#   xlab("First blood") +
#   ylab("First Champion kill")
```

We now predict on the test set.
```{r}
probsPredict <- predict(modelTune,
                        newdata = testing.noId,
                        type = "prob",
                        na.action = na.pass)

# Use thresold for division between the two categories
modelPredict <- probsPredict %>%
  mutate(winner = ifelse(red > rocCurve.thresold, "red", "blue")) %>%
  mutate(winner = as.factor(winner))

par(mfrow = c(1,1))
qplot(modelPredict$winner)

# Check AUC
rocCurve   <- roc(response = testing$winner,
                  predictor = modelPredict$red,
                  levels = rev(levels(testing$winner)))
rocCurve.auc <- as.numeric(rocCurve$auc)
rocCurve.auc
```

We look at misclassified.
```{r}
# Identify misclassified examples
# modelPredict <- modelPredict %>%
#   cbind(data.frame(true = testing$winner)) %>%
#   mutate(misclassified = winner != true)

testing.misclassified <- testing %>%
  cbind(data.frame(misclassified = modelPredict$winner != testing$winner))

testing.misclassified %>%
  merge(match.duration) %>%
  ggplot(aes(matchDuration, fill = misclassified)) + 
  geom_histogram(binwidth = 2) +
  xlab("Duration") +
  ylab("Count") +
  labs(fill = "Misclassified")

ggsave("figures/misclassified.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

```{r}
ggplot(testing.misclassified, aes(CHAMPION_KILL, ELITE_MONSTER_KILL, 
                         color = winner, group = winner)) +
                         # color = misclassified, group = misclassified)) + 
  # geom_point(size = 2) +
  # geom_point(aes(size = misclassified), alpha = 0.5) +
  # geom_point(aes(shape = misclassified), alpha = 0.5, size = 3) +
  geom_point(aes(alpha = misclassified), size = 3) +
  scale_color_manual(values = c(gg_blue, gg_red)) +
  # geom_density2d(alpha = 1) +
  xlab(paste("Champion killed per min in first", tf, "min")) +
  ylab(paste("Elite monster killed per min in first", tf, "min"))

# ggplot(testing.misclassified, aes(firstBlood, firstDragon, 
#                          color = misclassified, group = misclassified)) + 
#   geom_point(size = 2, position = "jitter") +
#   scale_color_manual(values = c(gg_blue, gg_red)) +
#   xlab("First blood") +
#   ylab("First Champion kill")
```

Look at confusion matrix.
```{r}
CM <- confusionMatrix(modelPredict$winner, testing$winner, positive = "red")

# Turn accuracy information into data frame
CM.df <- data.frame(t(CM$overall)) %>%
  mutate(AccuracyRange = (AccuracyUpper - AccuracyLower)/2,
         Method = method,
         AUC = rocCurve.auc)
CM.df

# Plot accuracy with error bars
ggplot(CM.df, aes(x = method, Accuracy)) +
  geom_bar(stat = "identity", position = "stack", fill = gg_blue) + 
  expand_limits(y = c(0,1)) +
  geom_errorbar(aes(ymin = AccuracyLower, ymax = AccuracyUpper), 
                width = 0.25, color = gg_purple) +
  geom_hline(yintercept = 0.5, color = gg_purple) +
  coord_cartesian(ylim = c(0.5, 1), expand = TRUE) +
  ylab("Accuracy") +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank())
```

Plot confusion matrix.
```{r}
# Show confusion matrix as table
CM.count <- table(predict = modelPredict$winner, true = testing$winner)

attr(CM.count, "dimnames")$predict <- attr(CM.count, "dimnames")$predict %>%
  str_replace("red", "Predict Red") %>%
  str_replace("blue", "Predict Blue")
attr(CM.count, "dimnames")$true <- attr(CM.count, "dimnames")$true %>%
  str_replace("blue", "Win Blue") %>%
  str_replace("red", "Win Red")

# st <- c(CM.count["Predict Red","Win Red"] / 
#          (CM.count["Predict Red","Win Red"] + CM.count["Predict Blue","Win Red"]),
#        CM.count["Predict Blue","Win Blue"] / 
#          (CM.count["Predict Blue","Win Blue"] + CM.count["Predict Red","Win Blue"]))

# heat map with red and blue NOTE update numbers manually
p <- ggplot(melt(CM.count), aes(predict, true, fill = true)) + 
  geom_tile(aes(alpha = value)) +
  scale_y_discrete(labels = c("Win Red" = "Win Red","Win Blue" = "Win Blue")) +
  # scale_y_discrete(labels = c("Win Red" = "Win Red\n65%","Win Blue" = "Win Blue\n75%")) +
  geom_text(aes(fill = true, label = value)) +
  scale_fill_manual(values = c(gg_blue, gg_red)) 

print(p)

# Remove padding and decorations
p <- p +
  theme(axis.line = element_blank(),
        axis.ticks = element_blank(),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.position = "none",
        panel.background = element_blank(),
        panel.border = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        plot.background = element_blank())

print(p)
ggsave("figures/predict.png", width = 4, height = 4, units = "in", bg = "transparent")

# CM.percent <- round(CM.count/length(testing$winner) * 100)
# CM.percent[1,1] <- paste(as.character(CM.percent[1,1]), "%")
# CM.percent[1,2] <- paste(as.character(CM.percent[1,2]), "%")
# CM.percent[2,1] <- paste(as.character(CM.percent[2,1]), "%")
# CM.percent[2,2] <- paste(as.character(CM.percent[2,2]), "%")

# png("figures/predict.png", bg = "transparent", res = 300, width = 1600, height = 900)
# grid.newpage()
# grid.table(CM.percent)
# dev.off()
```

## Compare methods together

```{r}
trainPredict <- function(method) {

modelTune <- modelCV(method)

probsTrain <- predict(modelTune,
                      training,
                      type = "prob",
                      na.action = na.pass)

rocCurve   <- roc(response = training$winner,
                  predictor = probsTrain$red,
                  levels = rev(levels(training$winner)))

# Plot true positive rate (sensitivity) versus false positive rate (1-specificity)
rocCurve.data <- data.frame(FPR = 1-rocCurve$specificities, 
                            TPR = rocCurve$sensitivities) %>%
  mutate(Method = method)

rocCurve.best <- rocCurve %>%
  coords("best", ret = c("sensitivity", "specificity", "threshold")) %>%
  t() %>%
  as.data.frame()

rocCurve.thresold <- rocCurve.best$threshold

probsPredict <- predict(modelTune,
                        newdata = testing,
                        type = "prob",
                        na.action = na.pass)

# Use thresold for division between the two categories
modelPredict <- probsPredict %>%
  mutate(winner = ifelse(red > rocCurve.thresold, "red", "blue")) %>%
  mutate(winner = as.factor(winner))

# Check AUC
rocCurve   <- roc(response = testing$winner,
                  predictor = modelPredict$red,
                  levels = rev(levels(testing$winner)))
rocCurve.auc <- as.numeric(rocCurve$auc)
rocCurve.auc

CM <- confusionMatrix(modelPredict$winner, testing$winner, positive = "red")

# Turn accuracy information into data frame
CM.df <- data.frame(t(CM$overall)) %>%
  mutate(AccuracyRange = (AccuracyUpper - AccuracyLower)/2,
         Method = method,
         AUC = as.numeric(rocCurve.auc))

return(list(CM.df, rocCurve.data))
}

method.acc <- as.data.frame(CM.df)
method.roc <- rocCurve.data %>%
  as.data.frame() %>%
  mutate(Method = method)

for (i in 1:length(method.all)) {
  method <- method.all[i]
  
  tP <- trainPredict(method)
  CM.df <- tP[1]
  rocCurve.data <- tP[2]
  
  CM.df <- as.data.frame(CM.df)
  rocCurve.data <- as.data.frame(rocCurve.data)
  
  if (i == 1) {
    method.acc <- CM.df
    method.roc <- rocCurve.data
  } else {
    method.acc <- rbind(method.acc, CM.df)
    method.roc <- rbind(method.roc, rocCurve.data)
  }
}
```

```{r}
# Sort methods by accuracy
method.acc$Method <- 
  factor(method.acc$Method, levels =
           method.acc$Method[order(method.acc$Accuracy)])

# Plot accuracy with error bars
ggplot(method.acc, aes(x = Method, y = 100*Accuracy)) +
  geom_bar(stat = "identity", position = "stack", fill = gg_blue) + 
  geom_errorbar(aes(ymin = 100*AccuracyLower, ymax = 100*AccuracyUpper), 
                width = 0.25, color = gg_purple) +
  geom_hline(yintercept = 50, color = gg_purple) +
  expand_limits(y = c(0,100)) +
  coord_cartesian(ylim = c(50, 90), expand = TRUE) +
  ylab("Accuracy") +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank())

ggsave("figures/methods_acc.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```

```{r}
# Sort methods by accuracy
method.roc$Method <- 
  factor(method.roc$Method, levels =
           method.acc$Method[order(method.acc$Accuracy)])

method.roc %>%
  ggplot(aes(x = 100*FPR, y = 100*TPR)) + 
  geom_point(aes(color = Method), size = 0.5) +
  # geom_line(aes(color = Method), alpha = 0.5, size = 0.5) +
  # geom_smooth(color = roccol, se = FALSE, alpha = 0.25) +
  xlab("False Positive Rate") +
  ylab("True Positive Rate") +
  # geom_abline(intercept = 0, slope = 1, alpha = 0.5, color = gg_purple)
  geom_segment(x = 0, y = 0, xend = 100, yend = 100, color = gg_purple, 
               alpha = 0.5, size = 0.25)

ggsave("figures/methods_roc.png", width = 8, height = 4.5, units = "in", bg = "transparent")
```
