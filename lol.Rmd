...
timeline.surrender <- merge(match.duration, match.surrender) %>%
  ...
  mutate(eventType = ifelse(surrender, "SURRENDER", "CASTLE DESTROY"),
...
timeline <- rbind(timeline.monster, timeline.kill, timeline.maxlvl, timeline.surrender)
...
dislocations <- c(1.0, -1.0, 0.5, -0.5, 1.0, -1.0, 0.5)
