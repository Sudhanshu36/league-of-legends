...
```{r}
permatch.participants <- participants %>%
  as.data.frame() %>%
...
We look at number of kills versus creeps.
data <- merge(permatch.events, permatch.participants)
```{r}
ggplot(data, aes(CHAMPION_KILL, creepsPerMinDeltas, 
                 color = factor(winner), group = factor(winner))) + 
  geom_point(alpha = 0.2, size = 1) +
...
```{r}
data <- permatch.events
# data <- permatch.events.team
# data <- permatch.team
# data <- merge(permatch.events, permatch.team) %>% merge(permatch.participants) 
